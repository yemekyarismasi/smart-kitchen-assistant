...
export default function KitchenMode({ recipe, onClose }) {
  ...
  if (recipe.description) {
    playList.push({
      ...
      speakText: `Mutfak moduna hoş geldiniz şefim. ${recipe.title} tarifinin hikayesi: ${recipe.description}. Hazırsanız 'devam' diyerek malzemelere geçebilirsiniz.`
    });
  }
  ...
  if (!recipe.description) {
      ingredientsSpeakText += `Mutfak moduna hoş geldiniz şefim. ${recipe.title} için `;
  }
  ingredientsSpeakText += `Malzemeler: ${activeIngredients.map(formatIngredientForSpeech).join('. ')}. Hazırsanız 'devam' diyerek ilk adıma geçebiliriz.`;
  ...
  if (!hasPromptedMeasurementRef.current && measurementMode === 'grams' && (recipe.ingredients || []).some(i => i.toLowerCase().includes('g ') || i.toLowerCase().includes('gram') || i.toLowerCase().includes('gr '))) {
    ingredientsSpeakText = "Malzemeleri orijinal gramajıyla okuyorum. İsterseniz ekrandaki butondan veya 'Bardak Ölçüsü' diyerek ev tipi ölçülere geçebilirsiniz. " + ingredientsSpeakText;
  }
    ...
    playList.push({
      ...
      content: (
        <div className="flex flex-wrap justify-center gap-4 md:gap-6">
          <div className="bg-slate-800 p-4 md:p-6 rounded-2xl md:rounded-3xl text-center min-w-[120px] shadow-lg border border-slate-700"><span className="block text-slate-400 mb-2">Kalori</span><span className="text-2xl md:text-4xl font-black text-amber-400">{recipe.nutrition.calories} kcal</span></div>
          ...
          <div className="bg-slate-800 p-4 md:p-6 rounded-2xl md:rounded-3xl text-center min-w-[120px] shadow-lg border border-slate-700"><span className="block text-slate-400 mb-2">Yağ</span><span className="text-2xl md:text-4xl font-black text-rose-400">{recipe.nutrition.fat}g</span></div>
          <div className="bg-slate-800 p-4 md:p-6 rounded-2xl md:rounded-3xl text-center min-w-[120px] shadow-lg border border-slate-700"><span className="block text-slate-400 mb-2">Karb.</span><span className="text-2xl md:text-4xl font-black text-emerald-400">{recipe.nutrition.carbs}g</span></div>
        </div>
      ),
      speakText: `Besin Değerleri: ${recipe.nutrition.calories || 0} kalori, ${recipe.nutrition.protein || 0} gram protein, ${recipe.nutrition.fat || 0} gram yağ ve ${recipe.nutrition.carbs || 0} gram karbonhidrat.`
    });
  }

  playList.push({
    title: 'TEBRİKLER! 🎉',
    content: <p className="text-3xl md:text-5xl text-slate-300 leading-tight">Yemeğiniz hazır. Afiyet olsun.</p>,
    speakText: "Tarif tamamlandı. Ellerinize sağlık, afiyet olsun!"
  });
  ...
  const handleVoiceCommand = (command) => {
    if (command.includes('ileri') || command.includes('sonraki') || command.includes('devam') || command.includes('next')) {
      handleNext();
    } else if (command.includes('geri') || command.includes('önceki') || command.includes('back')) {
      handlePrev();
    } else if (command.includes('tekrar') || command.includes('tekrarla') || command.includes('ne dedin')) {
      readCurrent();
    } else if (command.includes('malzeme') || command.includes('neler lazım')) {
      const ingIndex = playList.findIndex(p => p.title === 'MALZEMELER');
      if (ingIndex !== -1) {
        setCurrentStepIndex(ingIndex);
        speak(playList[ingIndex].speakText);
      }
    } else if (command.includes('bardak ölçü') || command.includes('ev usulü') || command.includes('ev tipi') || command.includes('kaşık ölçü')) {
      const ingIndex = playList.findIndex(p => p.title === 'MALZEMELER');
      if (ingIndex !== -1) setCurrentStepIndex(ingIndex);
      handleMeasurementToggle('household', true);
    } else if (command.includes('gram ölçü') || command.includes('orijinal ölçü') || command.includes('gramaj')) {
      const ingIndex = playList.findIndex(p => p.title === 'MALZEMELER');
      if (ingIndex !== -1) setCurrentStepIndex(ingIndex);
      handleMeasurementToggle('grams', true);
    } else if (command.includes('kaç dakika') || command.includes('ne kadar süre') || command.includes('süre ne durumda') || command.includes('kalan süre')) {
      if (timerSeconds !== null && timerSeconds > 0) {
        const mins = Math.floor(timerSeconds / 60);
        const secs = timerSeconds % 60;
        let response = "Sürenizin bitmesine ";
        if (mins > 0) response += `${mins} dakika `;
        if (secs > 0) response += `${secs} saniye `;
        response += "kaldı şefim.";
        speak(response, () => {
        });
      } else {
        speak("Şu anda çalışan bir zamanlayıcı bulunmuyor şefim.");
      }
    } else if (command.includes('süre ver') || command.includes('zamanlayıcı') || command.includes('dakika süre') || command.includes('saniye süre')) {
      const matchMin = command.match(/(\d+)\s*dakika/);
      const matchSec = command.match(/(\d+)\s*saniye/);
      ...
      if (totalSeconds === 0) {
        const trNumbers = { 'bir': 1, 'iki': 2, 'üç': 3, 'dört': 4, 'beş': 5, 'altı': 6, 'yedi': 7, 'sekiz': 8, 'dokuz': 9, 'on': 10, 'on beş': 15, 'yirmi': 20, 'otuz': 30, 'kırk': 40, 'elli': 50, 'altmış': 60 };
        for (const [key, val] of Object.entries(trNumbers)) {
           if (command.includes(`${key} dakika`)) totalSeconds += val * 60;
           else if (command.includes(`${key} saniye`)) totalSeconds += val;
        }
      }

      if (totalSeconds === 0) {
        speak("Lütfen rakam belirterek söyleyin. Örneğin: 5 dakika süre ver.");
        return;
      }

      setTimerSeconds(totalSeconds);
      speak(`${totalSeconds >= 60 ? Math.floor(totalSeconds / 60) + ' dakika' : totalSeconds + ' saniye'} süre başlatıldı.`);
      ...
      timerRef.current = setInterval(() => {
        setTimerSeconds(prev => {
          if (prev === null || prev <= 1) {
             if (timerRef.current) clearInterval(timerRef.current);
             speak("Belirttiğiniz süre doldu! Lütfen yemeğinizi kontrol edin.", () => {
                readCurrent();
             });
             return null;
          }
    ...
    } else if (command.includes('dur') || command.includes('bekle')) {
      pauseSpeech();
    } else if (command.includes('sürdür') || command.includes('devam et')) {
      resumeSpeech();
    } else if (command.includes('kapat') || command.includes('çık') || command.includes('tamamdır') || command.includes('stop')) {
      onClose();
    }
  };
  ...
  const handleMeasurementToggle = async (mode, triggerSpeech = false) => {
    ...
    if (mode === 'household' && householdIngredients.length === 0) {
      ...
      if (triggerSpeech) {
         speak("Ev tipi ölçüler hesaplanıyor, lütfen bekleyin...");
      }
      try {
        // B2B Demo Fake Conversion
        await new Promise(r => setTimeout(r, 1000));
        const enriched = (recipe.ingredients || []).map(i => i + " (Ev Ölçüsü)");
        setHouseholdIngredients(enriched);
        
        if (triggerSpeech) {
          const newSpeakText = `Ev tipi ölçülere geçildi. Yeni ölçülerle malzemeler: ${enriched.map(formatIngredientForSpeech).join('. ')}. Hazırsanız 'devam' diyerek ilk adıma geçebiliriz.`;
          speak(newSpeakText);
        }
      } catch (err) {
        console.error(err);
      ...
      const prefix = mode === 'household' ? "Ev tipi ölçülere geçildi. " : "Orijinal gramajlara dönüldü. ";
      const newSpeakText = `${prefix} Malzemeler: ${activeList.map(formatIngredientForSpeech).join('. ')}. Hazırsanız 'devam' diyerek ilk adıma geçebiliriz.`;
      speak(newSpeakText);
    }
  };
      ...
      {showIntro && (
        <div className="absolute inset-0 z-50 bg-slate-900/95 backdrop-blur-md overflow-y-auto flex flex-col items-center p-4 sm:p-8 text-center animate-fade-in">
          <div className="bg-slate-800 p-6 sm:p-12 rounded-[2rem] sm:rounded-[3rem] border border-slate-700 shadow-2xl max-w-2xl w-full my-auto">
            ...
            <h2 className="text-3xl md:text-5xl font-black text-amber-400 mb-6">Mutfak Modu</h2>
            <p className="text-xl text-slate-300 mb-8 leading-relaxed">
              Yemek yaparken elleriniz hamurluyken ekrana dokunmanıza gerek yok! Sesli komutlarla asistanı yönetebilirsiniz.
            </p>
            <div className="bg-slate-900 rounded-3xl p-6 mb-8 text-left border border-slate-700">
              <h3 className="font-bold text-emerald-400 mb-4 text-lg">Sesli Komutlar:</h3>
              <ul className="space-y-3 text-slate-300">
                <li><span className="text-amber-400 font-bold">"Devam"</span> veya <span className="text-amber-400 font-bold">"Next"</span> - Sonraki adıma geçer</li>
                <li><span className="text-amber-400 font-bold">"Geri"</span> veya <span className="text-amber-400 font-bold">"Back"</span> - Önceki adıma döner</li>
                <li><span className="text-amber-400 font-bold">"Tekrarla"</span> - Bulunduğunuz adımı baştan okur</li>
                <li><span className="text-amber-400 font-bold">"5 dakika süre ver"</span> - Zamanlayıcı başlatır</li>
                <li><span className="text-amber-400 font-bold">"Kapat"</span> - Çıkış yapar</li>
              </ul>
            </div>
            <button 
      ...
      <div className="mt-auto border-t border-slate-800 pt-6 flex flex-col items-center">
        ...
        <div className="mb-6 flex flex-col items-center gap-2">
          ...
          <span className={`font-bold text-lg ${isListening ? 'text-red-400' : 'text-slate-500'}`}>
            {isListening ? 'Sizi Dinliyor...' : 'Mikrofon Kapalı'}
          </span>
          {transcript && (
             <span className="text-slate-400 text-sm mt-1">Son komut: "{transcript}"</span>
          )}
        </div>
